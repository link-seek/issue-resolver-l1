...
## 你的任务
0. 先读「Human Steer / 根因分组」（如无则跳过）：
   - **Steer 至上**：输入中如有 `## Human Steer` 节，它是最高优先级指令。
     与下方默认策略冲突时（如 steer 要求删代码而下方要求最小改动），**遵 steer**，
     并在回复开头声明「按 Human Steer 执行，偏离默认策略之处如下」。
   - **根因合并**：动手前先把 findings 按根因分组，同一根因只修一次。
     回复先列分组（如「组1：前后端凭据链不对称 → state.rs/auth.ts/graphql-api.ts 共 7 条」），再施工。
   - **减法优先**：一致性类 findings 优先删冗余分支/回退链，而不是给各处加分支对齐。
     **禁止新增任何环境变量回退分支**（新的 `std::env::var` / `||` 兜底链）。
     `git diff` 自检时必须确认：净行数为负或零增长，且无新增 env 回退。
1. 理解审查反馈 — 识别提到的每个问题
2. 阅读相关文件理解现有代码
3. 修复审查中识别的每个问题
4. 运行测试验证修复:
...
## 跨文件一致性修复（防止振荡）
当 review-ai 报告「文件 A 的某值与文件 B 不一致」时，二选一（优先后者）：
- （a）砍到只剩一处：删掉冗余分支/回退链，让单一来源成为唯一真相；
- （b）修改所有位置使它们一致（仅当删除不可行时，如外部契约约束）。
...
1. **诊断** — 根据注入的 E2E 失败详情定位问题：
   - `GraphQL errors detected` → 查后端 resolver，用 introspection 确认 schema：
     ...
     curl -s http://localhost:8080/graphql -H 'Content-Type: application/json' \
       -d '{"query":"{ __type(name:\"Query\") { fields { name } } }"}' | python3 -m json.tool
     ```
   - `locator timeout` → 查前端组件是否渲染，检查 selector 是否匹配实际 DOM
